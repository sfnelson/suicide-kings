...
public class GroupService extends Locator<Group, Long> {
    ...
    @SuppressWarnings("unchecked")
    public Group findGroup(String name, String realm, String server) {
        EntityManager em = EMF.get().createEntityManager();
        try {
            ...
            for (Group g: groups) {
                if (!g.getServer().getRealm().equals(realm)) continue;
                if (!g.getServer().getServer().equals(server)) continue;
                matches.add(g);
            }
    ...
    public void createGroup(String name, String realm, String server) {
        if (name == null || realm == null || server == null) {
            throw new RuntimeException("invalid group parameters");
        }

        Group group = findGroup(name, realm, server);
        ...
        group = new Group();
        group.setName(name);
        group.setServer(new Server(realm, server));
